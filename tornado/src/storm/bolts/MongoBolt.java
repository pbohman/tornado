		...
		if (shouldActOnInput(input)) {
			...
			if (dbObject != null) {
				try {
					...
					// save wallclock time for performance measurements
					BasicDBObjectBuilder dbObjectBuilder = new BasicDBObjectBuilder();
					long wallclock = new Date().getTime();
					dbObjectBuilder.append("wallclock", wallclock);
					mongoDB.getCollection(collectionName + "_wallclock").save(dbObjectBuilder.get(), new WriteConcern(1));
					collector.ack(input);
